...
class MainWindow(ctk.CTk):
    def __init__(self, user_data):
        ...
        self.title(f"🏢 MINI ERP SUPERMERCADO V9 - Usuario: {user_data['username']} ({user_data['rol']})")
    ...
    def create_sidebar(self):
        ...
        self.sidebar.grid_rowconfigure(13, weight=1) # Spacer

        self.logo_label = ctk.CTkLabel(self.sidebar, text="🏢 MINI ERP\nJPV V9", font=Fonts.TITLE, text_color=Colors.TEXT)
        ...
        self.create_nav_button("📝 Notas de Crédito", self.show_credit_notes, 4)
        
        # Separator
        ttk_sep = ctk.CTkLabel(self.sidebar, text="📁 --- MAESTROS ---", text_color="gray")
        ttk_sep.grid(row=5, column=0, pady=5)
        
        self.create_nav_button("👥 Clientes", self.show_clients, 6)
        self.create_nav_button("📦 Productos", self.show_products, 7)
        self.create_nav_button("👔 Vendedores", self.show_sellers, 8)
        
        if self.user_data['rol'] == 'admin':
            self.create_nav_button("🛡️ Usuarios", self.show_users, 9)
            self.create_nav_button("🧩 Maestros Auxiliares", self.show_aux, 10)
            self.create_nav_button("📊 Reportes Avanzados", self.show_reports, 11)
        
        # Spacer
        self.btn_logout = ctk.CTkButton(self.sidebar, text="🚪 Cerrar Sesión", fg_color=Colors.DANGER, command=self.logout)
        self.btn_logout.grid(row=14, column=0, padx=20, pady=20)
